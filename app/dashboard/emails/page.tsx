    ...
    <div className="w-full min-h-screen bg-white p-8">
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500">Total Emails sent : 120</p>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          ...
          {/* Table header */}
          <div className="grid grid-cols-5 bg-gray-100 text-sm font-medium text-gray-700 border-t border-b border-gray-200">
          ...
          {emails.map((email) => (
            <div key={email.id} className="grid grid-cols-5 border-b border-gray-200 hover:bg-gray-50">
              <div className="px-6 py-4 text-gray-500 text-sm">{email.emailAddress}</div>
              <div className="px-6 py-4 text-gray-900 font-medium">{email.subject}</div>
              <div className="px-6 py-4 text-gray-500 text-sm truncate">{email.email}</div>
              <div className="px-6 py-4">
                {email.attachedDocuments.map((doc, index) => (
                  <div key={index} className="flex items-center text-gray-500 text-sm mb-1">
                    <File size={16} className="mr-2 text-gray-400" />
                    {doc}
                  </div>
                ))}
              </div>
              <div className="px-6 py-4 text-gray-500 text-sm">{email.sentOn}</div>
            </div>
          ))}
