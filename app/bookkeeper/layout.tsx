...
import {
  FileText, CreditCard, File, Video, Mail, Bell, ChevronRight, Menu, X,
} from 'lucide-react';
import Image from 'next/image';
interface BookkeeperLayoutProps {
  children: ReactNode
}

export default function BookkeeperLayout({ children }: BookkeeperLayoutProps) {
  ...
  const navItems = [
    { href: '/bookkeeper', icon: FileText, label: 'Transaction recording' },
    { href: '/bookkeeper/accounts-payable', icon: CreditCard, label: 'Accounts payable' },
    { href: '/bookkeeper/reconciliation', icon: File, label: 'Reconciliation' },
    { href: '/bookkeeper/basic-invoicing', icon: File, label: 'Basic invoicing' },
    { href: '/bookkeeper/education-content', icon: Video, label: 'Education Content' },
    { href: '/bookkeeper/emails', icon: Mail, label: 'Emails' },
    { href: '/bookkeeper/notifications', icon: Bell, label: 'Notifications' },
  ];
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="p-4 md:p-6">
        ...
        <Link href="/bookkeeper/profile" onClick={() => isMobile && setIsSidebarOpen(false)}>
          <div className="p-4 border-t border-[#3A4366] flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm font-medium">Bookkeeper</p>
              <p className="text-xs text-black">bookkeeper@click.com</p>
            </div>
