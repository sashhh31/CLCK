...
export interface PricingPlanLoginRedirect {
  title: string
  description: string
  price: number
  ctaText: string
...
export default function PricingCardsLoginRedirect({ pricingPlans }: { pricingPlans: PricingPlanLoginRedirect[] }) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 lg:gap-10 w-full max-w-[1000px]">
      {pricingPlans.map((plan, index) => (
        ...
        >
          ...
          <p className={`text-sm my-3 md:my-4 ${plan.highlightPlan ? 'text-white' : 'text-gray-300'}`}>{plan.description}</p>
          <div className="mb-4">
          ...
          <div className="mb-4">
            <span className={`text-sm mb-2 block ${plan.highlightPlan ? 'text-white' : 'text-gray-300'}`}>{plan.title} plan features</span>
